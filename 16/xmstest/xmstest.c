...
int main(void)
{
      ...
      printf("There are %lu extended memory bytes available\n", XMScoreleft());
      ...
      printf("Handle 1 is %u and represents %lu bytes\n", handle1, BlockSize);
      printf("There are %lu extended memory bytes available\n", XMScoreleft());
      ...
      printf("Handle 2 is %u and represents %lu bytes\n", handle2, BlockSize);
      printf("There are %lu extended memory bytes available\n", XMScoreleft());
      ...
      /* buf1 data shoulu now == buf2 data */
      ...
      XMSfree(handle2);
      printf("There are %lu bytes available after freeing handle 2\n",
             XMScoreleft());

      XMSfree(handle1);
      printf("There are %lu bytes available after freeing handle 1\n",
             XMScoreleft());
